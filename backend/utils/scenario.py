...
#콘텐츠 생성
def gen_contents(user_topic_input,topic,description,total_beats):
    from langchain import PromptTemplate
    from langchain_core.output_parsers import StrOutputParser
    from langchain_openai import ChatOpenAI
    llm = trranslate_model()
...
"""
    ...
    prompt = PromptTemplate.from_template(template)
    chain = prompt | llm | StrOutputParser()
    return chain.invoke({"user_topic_input": user_topic_input, "topic": topic, "description": description,"total_beats":total_beats})
...
def gen_dalle_first(width,height,prompt,background,save_dir):
...
#연속이미지 생성함수
def gen_dalle_series(width,height,prompt,background,previous_img,save_dir):
    client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
